...
export type ExperimentFlow = { id: string; title: string; eyebrow: string; summary: string; status: string; note: string; record: string; height?: number; methods?: ExperimentMethod[]; legend?: { main: string; loop: string; finish: string }; nodes: FlowNode[]; routes: FlowRoute[]; sequence: string[] };
...
export const myReActAgentFlow: ExperimentFlow = {
  ...
  legend: { main: '动作与分支', loop: 'Observation 回环', finish: '合法 Finish' },
  note: '演示合法 Finish 与两种反馈回路；空响应或步数耗尽时返回失败提示。',
  record: '2026.09.09 · 真实 API 完成计算工具与搜索工具两条闭环；搜索答案未补造 Observation 中不存在的官网地址。',
  nodes: [
    { id: 'prompt', label: '构造增强 Prompt', tag: 'CONTEXT', x: 20, y: 64, detail: '注入工具清单、用户问题和当前执行历史，并要求每次只输出 Thought 与 Action。', file: '7-experiment/agents/my_react_agent.py · MY_REACT_PROMPT' },
    { id: 'model', label: '模型生成动作', tag: 'THINK', x: 205, y: 64, detail: '每一步调用 LLM；空响应会立即停止，非空响应交给增强解析器。', file: '7-experiment/agents/my_react_agent.py · run' },
    { id: 'parse', label: '健壮解析 Action', tag: 'PARSE', x: 390, y: 64, detail: '容忍代码围栏、加粗、多行 Thought、中英文标签与动作尾注，再提取 工具名[参数]。', file: '7-experiment/agents/my_react_agent.py · _parse_output' },
    { id: 'tool', label: '执行工具', tag: 'ACTION', x: 390, y: 200, detail: '合法的非 Finish 动作交给 ToolRegistry，执行结果作为 Observation 写入当前历史。', file: '7-experiment/agents/my_react_agent.py · execute_tool' },
    { id: 'repair', label: '格式自纠', tag: 'REPAIR', x: 205, y: 200, detail: 'Action 无效或 Finish 为空时不直接退出，而是把明确格式要求作为 Observation 回灌。', file: '7-experiment/agents/my_react_agent.py · self-correction' },
    { id: 'finish', label: '返回受约束答案', tag: 'FINISH', x: 625, y: 64, kind: 'finish', detail: '合法 Finish 保存历史并返回；提示词要求具体事实只能来自本轮 Observation。', file: '7-experiment/agents/my_react_agent.py · Finish' },
  ],
  routes: [
    { id: 'prompt-model', from: 'prompt', to: 'model', d: 'M 160 96 H 205', kind: 'main' },
    { id: 'model-parse', from: 'model', to: 'parse', d: 'M 345 96 H 390', kind: 'main' },
    { id: 'parse-finish', from: 'parse', to: 'finish', d: 'M 530 96 H 625', kind: 'finish', label: '合法 Finish', x: 577, y: 82 },
    { id: 'parse-tool', from: 'parse', to: 'tool', d: 'M 475 128 V 200', kind: 'main', label: '工具动作', x: 512, y: 168 },
    { id: 'parse-repair', from: 'parse', to: 'repair', d: 'M 420 128 V 152 Q 420 162 410 162 H 285 Q 275 162 275 172 V 200', kind: 'main', label: '格式无效 / 空 Finish', x: 340, y: 149 },
    { id: 'tool-prompt', from: 'tool', to: 'prompt', d: 'M 460 264 V 294 Q 460 304 450 304 H 80 Q 70 304 70 294 V 128', kind: 'loop', label: '工具结果 · Observation', x: 270, y: 325 },
    { id: 'repair-prompt', from: 'repair', to: 'prompt', d: 'M 205 232 H 140 Q 130 232 130 222 V 128', kind: 'loop', label: '格式纠错反馈', x: 176, y: 184 },
  ],
...
export const myReflectionAgentFlow: ExperimentFlow = {
  ...
  status: '已运行 · 3 次真实模型调用', height: 345,
  methods: [
  ...
  legend: { main: '生成、评审与改进', loop: '下一轮反思', finish: '返回条件' },
  note: '流程演示包含提前停止、轮数耗尽与零轮迭代；不代表一次运行同时经过所有分支。',
  record: '2026.09.09 · 真实 API 完成初始回答、反思、改进 3 次调用；5 项离线测试覆盖提前停止、零轮迭代、误判防护与异常响应。',
  nodes: [
    { id: 'input', label: '校验并重置记忆', tag: 'TASK', x: 20, y: 64, detail: '拒绝空任务；每次 run 新建 ReflectionMemory，使不同任务的中间轨迹彼此隔离。', file: '7-experiment/agents/my_reflection_agent.py · run' },
    { id: 'initial', label: '生成初始回答', tag: 'INITIAL', x: 205, y: 64, detail: '使用 initial 模板调用 LLM，保存第一版 execution；max_iterations 为 0 时直接返回，不调用反思。', file: '7-experiment/agents/my_reflection_agent.py · initial' },
    { id: 'reflect', label: '反思当前回答', tag: 'REFLECT', x: 390, y: 64, detail: '读取最近一次 execution，结合原始任务生成反馈，并以 reflection 类型写入轨迹。', file: '7-experiment/agents/my_reflection_agent.py · reflect' },
    { id: 'check', label: '判断停止条件', tag: 'CHECK', x: 575, y: 64, detail: '只有完整反馈恰为“无需改进”或英文等价短语才提前停止，普通包含该词的反馈不会误判。', file: '7-experiment/agents/my_reflection_agent.py · _is_no_improvement' },
    { id: 'refine', label: '按反馈改进', tag: 'REFINE', x: 390, y: 210, detail: '把任务、上一版回答和反馈填入 refine 模板，生成新回答并追加为 execution。', file: '7-experiment/agents/my_reflection_agent.py · refine' },
    { id: 'finish', label: '返回最终答案', tag: 'RETURN', x: 575, y: 210, kind: 'finish', detail: '无需改进或达到 max_iterations 后，取最近回答写入 Agent 历史并返回。', file: '7-experiment/agents/my_reflection_agent.py · history' },
  ],
  routes: [
    { id: 'input-initial', from: 'input', to: 'initial', d: 'M 160 96 H 205', kind: 'main' },
    { id: 'initial-reflect', from: 'initial', to: 'reflect', d: 'M 345 96 H 390', kind: 'main' },
    { id: 'reflect-check', from: 'reflect', to: 'check', d: 'M 530 96 H 575', kind: 'main' },
    { id: 'check-finish', from: 'check', to: 'finish', d: 'M 715 96 H 750 Q 760 96 760 106 V 232 Q 760 242 750 242 H 715', kind: 'finish', label: '无需改进', x: 742, y: 174 },
    { id: 'check-refine', from: 'check', to: 'refine', d: 'M 615 128 V 162 Q 615 172 605 172 H 500 Q 490 172 490 182 V 210', kind: 'main', label: '需要改进', x: 550, y: 160 },
    { id: 'refine-reflect', from: 'refine', to: 'reflect', d: 'M 420 210 V 128', kind: 'loop', label: '下一轮', x: 382, y: 174 },
    { id: 'refine-finish', from: 'refine', to: 'finish', d: 'M 530 242 H 575', kind: 'finish', label: '达上限', x: 552, y: 228 },
    { id: 'initial-finish', from: 'initial', to: 'finish', d: 'M 275 128 V 300 Q 275 310 285 310 H 635 Q 645 310 645 300 V 274', kind: 'finish', label: '零轮迭代 · 直接返回初始回答', x: 465, y: 332 },
  ],
